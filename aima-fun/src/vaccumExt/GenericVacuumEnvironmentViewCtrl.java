 ...
 */
public class GenericVacuumEnvironmentViewCtrl<T> extends SimpleEnvironmentViewCtrl {

	protected GridPane envStateView;
	protected List<T> locations;
	private Agent agentInAction;

	public GenericVacuumEnvironmentViewCtrl(StackPane viewRoot) {
		super(viewRoot);
	...
	public void initialize(Environment env) {
		if (env instanceof GenericVacuumEnvironment) {
			this.locations = ((GenericVacuumEnvironment<T>) env).getLocations();
			...
			for (T loc : locations) {
				...
				pane.setTop(new Label(loc.toString()));
	...
	protected void updateEnvStateView(Environment env) {
		if (env instanceof GenericVacuumEnvironment) {
			GenericVacuumEnvironment<T> vEnv = (GenericVacuumEnvironment<T>) env;
			for (T loc : locations) {
	...
	private BorderPane getLocPane(T location) {
